...
* disroot.org - The server refused to sign the manifesto, because it considered that the manifesto would hit the reputation of the server as a server protesting against any manifestos. Nevertheless, the server 
perform the manifest according to the administrator (LOL)
*  blabber.de - The administrator says that unverified certificates are safer than verified certificates. To verify unverified certificates, he suggested using manual verification through email with each with tens of thousands of servers. (WAT?)
